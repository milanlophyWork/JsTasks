export function polynomial(degree:number, coefficients:number[], x:number){
    if(typeof degree !== 'number' || isNaN(degree) || !Array.isArray(coefficients) || 
    coefficients.length === 0 || typeof x !== 'number' || isNaN(x)) return 'invalid input'

    // console.log('Coefficients must be from higher order to lower order')
    if(coefficients.length !== degree + 1) return 'Enter correct number of coefficients from higher order to lower order'
    let sum:number = 0
    let temp:number
